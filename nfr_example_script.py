...
def nfr_example_simple():
    ...
    if os.path.isdir(nessus_scan_files):
        ...
        extension = '*.nessus'
        list_of_source_files = glob.glob(nessus_scan_files + os_separator + '**' + os_separator + extension,
                                         recursive=True)
        ...
        if os.path.isfile(nessus_scan_file):
            ...
            try:
                ...
                for report_host in nfr.scan.report_hosts(root):
                    ...
                    print(f'  Host scan time START - END (ELAPSED): '
                          f'{report_host_scan_time_start} - {report_host_scan_time_end} '
                          f'({report_host_scan_time_elapsed})')
                    print(f'  Critical/High/Medium/Low/None findings: {report_host_critical}/{report_host_high}/'
                ...
                print(f'\nUps... ERROR occurred. \n\n {str(e)}')
                traceback.print_exc()
                print(f'ERROR Parsing [{str(row_index+1)}/{str(len(list_of_source_files))}] nessus files')

        else:
            print(f'Ups.. {nessus_scan_file} does not exist in current directory: {os.getcwd()}')
...
def main():
    ...
    print(f'This is example script for {app_name} {app_version} {app_version_release_date}\n')
